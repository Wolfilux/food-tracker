...
test("propagates explicit years across named-month ranges and comparisons", () => {
  ...
  const relativeYearBoundary = resolveExplicitAnalysisPlan(
    "Wie lief es von November letzten Jahres bis Februar?",
    options,
  );
  ...
  assert.deepEqual(relativeYearBoundary.periods.map(({ from, to }) => ({ from, to })), [{
    from: "2025-11-01",
    to: "2026-02-28",
  }]);
});
...
test("uses eight weeks for an unclear weight trend and four weeks otherwise", () => {
  ...
  assert.equal(hasUnresolvedAnalysisTimeReference("Wie war es im Juni?"), false);
  assert.equal(hasUnresolvedAnalysisTimeReference("Wie war es seit KW 20?"), true);
  assert.equal(hasUnresolvedAnalysisTimeReference("Wie war es von KW 20 bis 22?"), true);
  assert.equal(hasUnresolvedAnalysisTimeReference("Wie war es von KW 20 bis KW 22?"), true);
